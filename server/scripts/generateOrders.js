...
const generateOrders = async (ordersPerRegion = 5000) => {
  try {
    ...
    for (let i = 0; i < orders.length; i += batchSize) {
      ...
      console.log(
        `Inserted ${Math.min(i + batchSize, orders.length)}/${orders.length} orders`
      );
    }

    console.log(
      `Successfully generated ${orders.length} orders across ${Object.keys(REGION_CONFIG).length} regions`
    );
  } catch (err) {
